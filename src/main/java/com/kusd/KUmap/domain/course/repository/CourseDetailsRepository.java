...
public interface CourseDetailsRepository extends JpaRepository<CourseDetails, String> {
    ...
    @Query("SELECT c FROM CourseDetails c WHERE (c.competencyInCourse.competencyCode1 = :cmptCode OR c.competencyInCourse.competencyCode2 = :cmptCode OR c.competencyInCourse.competencyCode3 = :cmptCode) AND c.applicationStatusCode = '승인'")
    List<CourseDetails> findAllByCompetencyCode(@Param("cmptCode") String cmptCode);

    @Query("SELECT c FROM CourseDetails c WHERE c.addInformation.openingSubjectCode = :openingSubjectCode AND c.applicationStatusCode = '승인'")
